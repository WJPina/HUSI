...
########################### Valid model in independent dataset ############################
x_valid = fread("valid_TPM_dataset.tsv") %>% 
    filter( `Gene Name` %in% names(mm_l2$w) ) %>% 
    column_to_rownames("Gene Name") %>% as.matrix

x_valid_metadata = data.frame( sample = colnames(x_valid) ) %>% 
    mutate( condition = if_else( grepl("Allstars|Water|Hs", sample), "senescent", "other" ) ) 
# Discard samples with low read counts
fread("filereport_read_run_PRJNA395386_tsv.txt", select = c("sample_title", "read_count"))[read_count<1e6]
# valid by OCLR models
# L2-norm
w = mm_l2$w[ rownames(x_valid) ]
s_valid = apply( x_valid, 2, function(z) {cor(z, w, method = "sp", use = "complete.obs")} )
s_valid %>% data.frame(score=.) %>% rownames_to_column("sample") %>% 
    inner_join( x_valid_metadata, by="sample" ) %>% 
    filter( !sample %in% fread("filereport_read_run_PRJNA395386_tsv.txt")[read_count<1e6]$sample_title ) %>% 
    mutate( condition = case_when(
        grepl("Allstars|Water|Hs|Extras[5-8]", sample) ~ "senescent", 
        grepl("Noninduced|Extras[1-4]", sample) ~ "proliferating", 
        TRUE ~ "other"
    ) ) %>% 
    group_by(condition) %>% summarise(max=max(score), min=min(score))
# L1-norm
apply( x_valid, 2, function(z) {cor(z, mm_l1$w[ rownames(x_valid) ], method = "sp", use = "complete.obs")} ) %>% 
    data.frame(score=.) %>% rownames_to_column("sample") %>% 
    inner_join( x_valid_metadata, by="sample" ) %>% 
    filter( !sample %in% fread("data/valid_dataset/filereport_read_run_PRJNA395386_tsv.txt")[read_count<1e6]$sample_title ) %>% 
    mutate( condition = case_when(
        grepl("Allstars|Water|Hs|Extras[5-8]", sample) ~ "senescent", 
        grepl("Noninduced|Extras[1-4]", sample) ~ "proliferating", 
        TRUE ~ "other"
    ...
    group_by(condition) %>% summarise(max=max(score), min=min(score))
########################### Enrichement of model signature ############################
# GO
library(org.Hs.eg.db)
l1_enrichGO = mm_l1$w %>% data.frame(a=.) %>% rownames_to_column("gene") %>% filter(a!=0) %>% .$gene %>% 
    clusterProfiler::enrichGO(OrgDb = org.Hs.eg.db, ont = "ALL", pool = T, keyType = "SYMBOL", pvalueCutoff = 1,qvalueCutoff = 1)
as.data.frame(l1_enrichGO)
# singscore
suppressPackageStartupMessages(library(singscore))
genesetC = getGmt("data/senescence_geneset.gmt", geneIdType = SymbolIdentifier())
s_singscore = SummarizedExperiment::SummarizedExperiment(assays = x_valid) %>%
    rankGenes() %>% 
    # simpleScore(upSet = genesetC[[3]], knownDirection = F) %>% 
    # simpleScore(upSet = genesetC[[6]], knownDirection = F) %>%
    simpleScore(upSet = genesetC[[7]], knownDirection = F) %>%
    # simpleScore(upSet = genesetC[[5]], downSet = genesetC[[4]]) %>%
    # simpleScore(upSet = genesetC[[1]], downSet = genesetC[[2]]) %>%
...
s_singscore %>% 
    ...
    group_by(condition) %>% summarise(max=max(TotalScore), min=min(TotalScore))